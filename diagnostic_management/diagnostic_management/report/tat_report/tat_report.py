...
from frappe.utils import getdate, flt, time_diff_in_hours, get_datetime
...
def get_data(filters):
    ...
    # Default expected TAT in hours (24 hours if not specified)
    ...
    # Add expected TAT from settings or default
    for row in data:
    ...
    for row in data:
        ...
        if row.result_date and row.creation_date:
            # Calculate hours from creation to result
            ...
            # Still pending - check if already breached
            if row.expected_tat:
                creation_dt = get_datetime(row.creation_date)
...
def get_conditions(filters):
    ...
    # Branch lives on Patient — joined when this filter is set.
...
def get_summary(data):
    ...
    if completed_with_tat:
        avg_tat = sum(d.get("actual_tat", 0) for d in completed_with_tat) / len(completed_with_tat)
    ...
    return [
        ...
        },
        {
            ...
            "datatype": "Percent",
            "indicator": "green" if compliance_rate >= 90 else "orange" if compliance_rate >= 70 else "red"
        }
    ]
